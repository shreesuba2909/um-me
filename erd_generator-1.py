...
def generate_erd_ldm(sql_text: str,
    ...
    # Assign stable IDs for every entity, attribute, identifier (PK), and relationship
    entity_ids:    Dict[str, str]  = {}
    attr_ids:      Dict[Tuple, str] = {}
    ident_ids:     Dict[str, str]  = {}   # primary identifier per entity
    rel_ids:       List[Tuple]     = []
    ...
    for tname, table in tables.items():
        ...
        # Attributes (logical equivalent of columns)
        ...
        for col in table.columns:
            ...
            # Mandatory = 1 for NOT NULL or PK columns
            _sub(a_el, "a:Mandatory",        text="1" if (not col.is_nullable or col.is_primary_key) else "0")

            if col.is_primary_key:
                pk_attr_ids.append(aid)

        # Primary Identifier (logical equivalent of PK key)
        if pk_attr_ids or table.primary_keys:
            ...
            # PrimaryIdentifier flag = 1 marks this as the main identifier
    ...
    # ── Relationships ─────────────────────────────────────────────────────────
    if rel_ids:
        rels_coll = _sub(model_el, "c:Relationships")

        for fk, rid in rel_ids:
            if fk.from_table not in tables or fk.to_table not in tables:
                continue

            r_el      = _sub(rels_coll, "o:Relationship", Id=rid)
            rel_name  = f"{fk.to_table}_has_{fk.from_table}"
            rel_code  = f"R_{fk.from_table}_{fk.from_col}"

            _sub(r_el, "a:ObjectID",         text=_make_guid())
            _sub(r_el, "a:Name",             text=rel_name)
            _sub(r_el, "a:Code",             text=rel_code)
            _sub(r_el, "a:CreationDate",     text=now_ts)
            _sub(r_el, "a:Creator",          text="erd_generator")
            _sub(r_el, "a:ModificationDate", text=now_ts)
            _sub(r_el, "a:Modifier",         text="erd_generator")

            # Cardinality: parent side = 1,1  (one mandatory)
            #              child side  = 0,n  (zero-to-many)
            _sub(r_el, "a:Entity1ToEntity2", text=_CARDINALITY_MANY)  # parent → child
            _sub(r_el, "a:Entity2ToEntity1", text=_CARDINALITY_ONE)   # child  → parent

            # Entity1 = parent (the referenced / "one" side)
            ET.SubElement(
                _sub(r_el, "c:Entity1"),
                "o:Entity"
            ).set("Ref", entity_ids[fk.to_table])

            # Entity2 = child (the referencing / "many" side)
            ET.SubElement(
                _sub(r_el, "c:Entity2"),
                "o:Entity"
